...
function Create()
	Set("VisitorID","None")
	this.Tooltip = "tooltip_EmptyStaffGrave2"
end
...
function Update(timePassed)
	...
	if timeTot >= timePerUpdate then
		...
		--	if not Get("VisitorSpawned") then
		--		SpawnVisitor("Funeral")
		--	elseif not Get("VisitorLeaving") then
		--		this.CreateJob("VisitFuneral")
		--	else
...
function JobComplete_CoffinInHole()
	nearbyCoffins = Find(this,"GraveCoffinStaffLoaded2",3)
...
function JobComplete_BuryEntity()
	...
	for thatCoffin, dist in pairs(nearbyCoffins) do
		if thatCoffin.Id.i == this.Slot0.i then
			...
			newGrave = Object.Spawn("GraveStaffLoaded", this.Pos.x,this.Pos.y)
